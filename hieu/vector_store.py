...
import config
from rag import BM25Index, normalize_scores, rerank_results
...
class HybridStore:
    ...
    def _dedup(self, ranked: list[tuple[int, float]], limit: int) -> list[tuple[int, float]]:
        """Bo chunk gan trung lap (cosine > nguong) de context da dang hon."""
        if self.embeddings is None or config.DEDUP_THRESHOLD >= 1.0:
            return ranked[:limit]
        kept: list[tuple[int, float]] = []
        for index, score in ranked:
            emb = self.embeddings[index]
            if any(float(emb @ self.embeddings[j]) > config.DEDUP_THRESHOLD for j, _ in kept):
                continue
    ...
    def search_bm25(self, question: str, top_k: int) -> list[tuple[str, float]]:
        """Duong BM25 thuan — dung lam fallback khi embedding loi/thieu."""
        if not self.chunks:
            return []
        ranked = self._ensure_bm25().rank(question, top_k)
        results = [(self.chunks[i], score) for i, score in ranked]
    ...
    def search(self, query, top_k: int = 5) -> list[tuple[str, float]]:
        ...
        if backend in {"bm25", "tfidf"}:
            return self.search_bm25(str(query), top_k)

        if backend in {"sbert", "openai", "vector"}:
            ranked = self._dedup(self._vector_search(query, top_k * 2), top_k)
            results = [(self.chunks[i], score) for i, score in ranked]
        ...
        bm25_ranking = self._ensure_bm25().rank(question, candidate_k)

        if config.HYBRID_FUSION == "rrf":
            ranked = list(_rrf_fuse([vector_ranking, bm25_ranking], max(config.RRF_K, 1)).items())
        else:
            vector_norm = normalize_scores(dict(vector_ranking), higher_is_better=True)
            bm25_norm = normalize_scores(dict(bm25_ranking), higher_is_better=True)
        ...
        ranked = self._dedup(ranked, candidate_k)
        results = [(self.chunks[index], score) for index, score in ranked]
