"""LLM client goi Teacher proxy (OpenAI-compatible). Toi uu cho model yeu ~4k context."""
...
_WEAK_TOKENS = {
    ...
    "trong", "mot", "một", "cac", "các", "theo", "quy", "dinh", "định", "phap", "pháp",
    "luat", "luật", "thong", "thông", "tu", "tư", "nghi", "nghị", "noi", "nội",
}
...
def parse_answer(text: str) -> str:
    """Trich xuat 1 chu cai A/B/C/D tu output tho cua LLM."""
    if not text:
        return "A"
    s = text.strip().upper()
    if s in {"A", "B", "C", "D"}:
...
def _options_block(options: Mapping[str, str] | None) -> str:
    if not options:
        return ""
    return "\n".join(
        f"{key}. {clean_inline_text(value)}" for key, value in sorted(options.items()) if key in "ABCD"
    )
...
def score_options(question: str, context: str, options: Mapping[str, str] | None) -> dict[str, float]:
    """Cham diem tung phuong an theo do khop voi context (nguyen van, con so,
    dieu/khoan, overlap); cau hoi phu dinh (KHONG/tru/khac) thi dao tieu chi."""
    ...
    for label, option_text in sorted(options.items()):
        ...
        for window, window_canon in zip(windows, canonical_windows):
            ...
            article_hit = 1 if any(
                tok.startswith("dieu") or tok.startswith("khoan")
...
def heuristic_answer(question: str, context: str, options: Mapping[str, str] | None = None) -> str:
    """Fallback local neu proxy LLM loi/timeout."""
...
def compact_context(chunks: list[str] | str, max_chars: int | None = None) -> str:
    ...
    for chunk in chunks:
        ...
        if len(cleaned) > remaining:
            # Cat theo ranh gioi tu, khong dut giua tu.
...
def _should_override(scores: dict[str, float], llm_answer: str) -> str | None:
    """Heuristic de LLM khi bang chung trong context cuc ro (nguyen van + cach biet lon)."""
...
def ask_llm(question: str, context: str, options: Mapping[str, str] | None = None) -> str:
    options = dict(options or extract_options(question))
    scores = score_options(question, context, options)
    user = _build_user_prompt(question, context, options)
    # Retry trong ngan sach 60s cua Teacher: (LLM_RETRIES+1) lan * LLM_TIMEOUT.
    # Het luot van loi -> heuristic local, khong bao gio tra 500.
    ...
    for _ in range(attempts):
        try:
            ...
            if raw.strip():
                ...
                return _should_override(scores, parsed) or parsed
        except Exception:
            continue
...
def embed_openai(texts: list[str]) -> np.ndarray:
    """Goi OpenAI Embeddings qua proxy cua Teacher Server (neu RETRIEVER_BACKEND=openai)."""
    if not texts:
