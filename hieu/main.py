"""Student Server v5 - FastAPI: /upload + /ask cho Teacher Server.

Hop nhat: retrieval hybrid (v4) + answer cache xuong disk (v1) + persist VectorDB.
Toi uu cho LLM proxy ~4k context, tai lieu dai, 100 cau, 5 lan nop lap lai.
"""
...
def _retrieve(question: str, options: dict[str, str], top_k: int) -> list[tuple[str, float]]:
    # Query = than cau hoi (bo options) cho retrieval sach hon.
    retrieval_query = compose_retrieval_query(
        question, options=options, include_options=config.RETRIEVAL_INCLUDE_OPTIONS
    )
    if config.RETRIEVER_BACKEND in {"tfidf", "bm25"}:
...
def _safe_retrieve(question: str, options: dict[str, str], top_k: int) -> list[tuple[str, float]]:
    """Khong de /ask chet vi retrieval: embed loi (vd thieu model cache khi offline)
    thi fallback BM25 thuan; te nhat tra rong de LLM/heuristic tu xoay."""
    try:
        return _retrieve(question, options, top_k)
    except Exception:
        log.exception("Retrieve loi, fallback BM25 thuan")
        try:
            return store.search_bm25(question, top_k)
        except Exception:
            log.exception("BM25 fallback cung loi, tra context rong")
            return []


# ----------------- Answer cache (tai dung qua 5 lan nop lap lai) -----------------
...
async def upload(request: Request) -> UploadResponse:
    ...
    # Embedding la CPU-bound, day ra threadpool de khong nghen event loop.
    await run_in_threadpool(_index, chunks)
...
async def ask(request: Request) -> AskResponse:
    ...
    if len(store) == 0:
        ...
        answer = await run_in_threadpool(
            ask_llm, question, "(khong co tai lieu)", options
        )
        _save_cached_answer(question, options, answer, [])
        return AskResponse(answer=answer, sources=[])

    hits = await run_in_threadpool(_safe_retrieve, question, options, config.TOP_K)
    ...
    answer = await run_in_threadpool(ask_llm, question, context, options)
